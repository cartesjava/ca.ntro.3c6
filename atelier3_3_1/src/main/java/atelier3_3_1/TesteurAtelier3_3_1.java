...
public class TesteurAtelier3_3_1 extends PerformanceTestsDriver {
	...
	public TestParameters getTestParametersFor(String providerMethodPrefix, String methodName) {
		...
		if(methodName.equals("trouverIndicePourValeur")) {
			...
			testsParameters = new TestParameters((int) 1E7, (int) 1E8, 6, 0.6, false);
		}
	...
	public List<Object> generateArgumentsFor(String providerMethodPrefix, String methodName, int desiredInputSize) {
		...
		if(methodName.equals("trouverIndicePourValeur")) {
			...
			arguments.add(tableau.obtenirValeur(desiredInputSize-1));
	...
	public static Tableau<Integer> sortedArray(int size){
		...
		for(int i = 1; i < size; i++) {
			result[i] = result[i-1] + 1 + random.nextInt(5);
		}
